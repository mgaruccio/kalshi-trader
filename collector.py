...
from nautilus_trader.live.config import TradingNodeConfig, LiveDataClientConfig
...
class KalshiDiscoveryStrategy(Strategy):
    ...
    def _sync_discover_markets(self, loop):
        ...
        series_api = SeriesApi(self.provider.k_client)
        markets_api = MarketsApi(self.provider.k_client)
            ...
            while True:
                try:
                    ...
                    for s in series_list:
                        # Ensure it's a weather series and related to high temps
...
def main():
    ...
    config_obj = KalshiConfig(
        ...
        environment="prod",
        rest_host="https://api.elections.kalshi.com/trade-api/v2",
        ws_host="wss://api.elections.kalshi.com/trade-api/ws/v2",
    )
    ...
    # Notice we pass the *class*, not the instance, because Nautilus needs to instantiate it internally
